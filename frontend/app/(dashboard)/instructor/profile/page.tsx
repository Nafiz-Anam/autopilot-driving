    ...
    <div>
      ...
      <input name={name} value={value} onChange={onChange} type={type} readOnly={readOnly} placeholder={placeholder}
        className={cn(
          ...
          readOnly ? "bg-brand-surface text-brand-muted cursor-not-allowed"
  ...
  return (
    <div>
      ...
      <div className="relative">
        <input name={name} value={value} onChange={onChange} type={show ? "text" : "password"} placeholder={placeholder}
        ...
        {!confirmed ? (
          <div className="space-y-4">
            <div>
              ...
              <input value={input} onChange={(e) => setInput(e.target.value)} placeholder={PHRASE}
            ...
            <div className="flex gap-3">
              ...
              <button disabled={input !== PHRASE} onClick={() => setConfirmed(true)}
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <motion.div variants={itemVariants} className="bg-white rounded-2xl border border-brand-border shadow-sm p-6">
            ...
            <form onSubmit={handleProfileSave} className="space-y-4">
                ...
                <div className="flex gap-4">
                  {["manual", "automatic"].map((t) => (
                    <label key={t} className="flex items-center gap-2 text-sm cursor-pointer">
                      <input type="checkbox" checked={profile.transmission.includes(t)} onChange={() => toggleTransmission(t)}
              ...
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={profile.isFemale}
          ...
          <motion.div variants={itemVariants} className="bg-white rounded-2xl border border-brand-border shadow-sm p-6">
            ...
            <form onSubmit={handlePasswordSave} className="space-y-4">
              <PasswordInput label="Current Password" name="current" value={passwords.current} onChange={handlePasswordChange} placeholder="Enter current password" />
              <PasswordInput label="New Password" name="next" value={passwords.next} onChange={handlePasswordChange} placeholder="At least 8 characters" />
              <div>
                <PasswordInput label="Confirm New Password" name="confirm" value={passwords.confirm} onChange={handlePasswordChange} placeholder="Repeat new password" />
                <AnimatePresence>
                  {passwordMismatch && (
                    <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                      className="text-xs text-brand-red mt-1">Passwords do not match</motion.p>
                  )}
                  {passwordError && (
                    <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
                      className="text-xs text-brand-red mt-1">{passwordError}</motion.p>
                  )}
                </AnimatePresence>
              </div>
              <div className="flex items-center gap-4 pt-1">
                <button type="submit" disabled={!passwordMatch || !passwords.current || savingPassword}
                ...
                <AnimatePresence>
                  {passwordSaved && (
                    <motion.span initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0 }}
                      className="text-sm text-green-600 font-medium">✓ Password updated</motion.span>
                  )}
                </AnimatePresence>
              </div>
            </form>
