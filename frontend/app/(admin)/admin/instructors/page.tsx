              ...
              <>
                ...
                <div className="flex items-center justify-between mt-4">
                  ...
                  <button type="button" onClick={handleSaveSchedule} disabled={scheduleSaving}
                    className={cn("px-5 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2",
                      scheduleSaved ? "bg-green-500 text-white" : "bg-brand-red text-white hover:bg-brand-orange disabled:opacity-60")}>
                    {scheduleSaving ? <><span className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin" />Saving…</> : scheduleSaved ? <><Check className="w-3.5 h-3.5" />Saved!</> : "Save Schedule"}
                  </button>
                </div>
              </>
            )}
